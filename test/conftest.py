...
def Account():
    ...
    companycode = (''.join(random.choice(string.ascii_lowercase) for i in range(5)))
    # email = (''.join(random.choice(string.ascii_lowercase) for i in range(5))) + "@test.com"
    return(Username, Password, companycode)
...
def invalidAccount():
    ...
    companycode = ''.join((random.choice(string.ascii_letters) for x in range(7)))   
    return(Username, Password, companycode)